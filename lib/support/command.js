...
const _validateCmdExist = function() {
    ...
    if (this.varIsValid){
        ...
        if(compts._.has(this.config.cli_config.command, this.commandKey) == false){
            this.varIsValid = false;
...
class Command{
    ...
    executeCommand(){
        ...
        this.config.argv['argv']=compts._.to_array(compts._.getValue(compts._.limit(this.config.argv['argv'],1)));
